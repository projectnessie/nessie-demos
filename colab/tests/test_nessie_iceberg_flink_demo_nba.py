...
def test_notebook_output(notebook):
    ...
    assert_that(
        notebook.nb['cells'][8].outputs[0]["data"]["text/plain"]).contains(
    ...
    assert_that(
        notebook.nb['cells'][22].outputs[0]["data"]["text/plain"]).contains(
    ...
    assert_that(
        notebook.nb['cells'][34].outputs[0]["data"]["text/plain"]).contains(
        "59")

    # table_env.from_path('nessie.nba.`salaries@main`').select(lit(1).count).to_pandas()
    assert_that(
        notebook.nb['cells'][36].outputs[0]["data"]["text/plain"]).contains(
        "55")
